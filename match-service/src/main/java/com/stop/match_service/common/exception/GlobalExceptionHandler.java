package com.stop.match_service.common.exception;

import com.stop.match_service.common.error.CommonErrorCode;
import com.stop.match_service.common.error.ErrorCode;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ProblemDetail> handleMethodNotSupported(HttpRequestMethodNotSupportedException ex, HttpServletRequest req) {
        ...
        ProblemDetail pd = ProblemDetail.forStatusAndDetail(HttpStatus.METHOD_NOT_ALLOWED, "Bu endpoint icin HTTP metodu desteklenmiyor");
        pd.setProperty("code", CommonErrorCode.INVALID_REQUEST.getCode());
        pd.setInstance(URI.create(req.getRequestURI()));
    ...
    public ResponseEntity<ProblemDetail> handleInvalidSort(InvalidDataAccessApiUsageException ex, HttpServletRequest req) {
        log.warn("Invalid database usage or sort parameter: {}", ex.getMessage());

        ProblemDetail pd = ProblemDetail.forStatusAndDetail(HttpStatus.BAD_REQUEST, "Girdiginiz siralama parametresi veya veri kullanimi gecersiz.");
        pd.setProperty("code", "INVALID_SORT");
        pd.setInstance(URI.create(req.getRequestURI()));
        return ResponseEntity.badRequest().body(pd);
    }
    ...
    public ResponseEntity<ProblemDetail> handleTypeMismatch(MethodArgumentTypeMismatchException ex, HttpServletRequest req) {
        String message = String.format("'%s' parametresi icin gecersiz deger: '%s'", ex.getName(), ex.getValue());
    ...
    public ResponseEntity<ProblemDetail> handleNotReadable(HttpMessageNotReadableException ex, HttpServletRequest req) {
        ...
        ProblemDetail pd = ProblemDetail.forStatusAndDetail(HttpStatus.BAD_REQUEST, "Gecersiz veri formati veya tur hatasi. Lutfen girilen degerleri kontrol edin.");
        pd.setProperty("code", CommonErrorCode.INVALID_REQUEST.getCode());
        pd.setInstance(URI.create(req.getRequestURI()));
        return ResponseEntity.badRequest().body(pd);
    }
