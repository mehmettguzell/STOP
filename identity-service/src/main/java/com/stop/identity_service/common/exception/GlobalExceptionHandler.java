package com.stop.identity_service.common.exception;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ProblemDetail> handleValidation(MethodArgumentNotValidException ex, HttpServletRequest req) {
        String message = ex.getBindingResult().getFieldErrors().stream()
                ...
                .findFirst()
                .orElse("Gecersiz istek. Lutfen girilen degerleri kontrol edin.");
    ...
    public ResponseEntity<ProblemDetail> handleNotReadable(HttpMessageNotReadableException ex, HttpServletRequest req) {
        ...
        ProblemDetail pd = ProblemDetail.forStatusAndDetail(HttpStatus.BAD_REQUEST, "Gecersiz veri formati. Lutfen girilen degerleri kontrol edin.");
        pd.setProperty("code", CommonErrorCode.INVALID_REQUEST.getCode());
        pd.setInstance(URI.create(req.getRequestURI()));
        return ResponseEntity.badRequest().body(pd);
    }
